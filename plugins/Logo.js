...
const handler = async (m, {conn, args: [effect], text: txt, usedPrefix, command, name}) => {
  ...
  if (!EfectoLogoKasuma.find(v => (new RegExp(v.title, 'gi')).test(effect))) throw `El logo ${effect}No se encuentra en la lista.`
 try {
  ...
  const res = await maker(effectoSelect?.url, [...text]).catch(_ => { throw `Escriba un texto para hacer el logo.` })
...
await m.reply(`Debes usar el comando de la siguiente manera\n*${usedPrefix + command} efecto texto*\nEjemplo\n💫 *${usedPrefix + command} American-flag-3D KasumaBot*\n\nY si te dice que falta texto, es porque el logo es de 2 texto, ejemplo: *${usedPrefix + command} Ninja-Logo texto1|texto2*\nEjemplo *${usedPrefix + command} Ninja-Logo KasumaBot|KasumaBot*\n\n\n\nSi no funciona el comando reportelo de la siguiente manera: .reporte <texto>`)
console.log(e)}
}
handler.help = ['logos'];
