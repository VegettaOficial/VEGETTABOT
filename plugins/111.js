const handler = async (m, {conn, isAdmin, isOwner, args, usedPrefix, command}) => {
  ...
  if (isClose === undefined) {
   const caption = *FORMATO ERRONEO!!*
 
 ${usedPrefix + command} abrirgrupoen 1
 ${usedPrefix + command} cerrargrupoen 1
 *Ejemplo de uso:* *${usedPrefix + command} close 1* 
 Para que el grupo este cerrado una hora.*;
  ...
  await conn.groupSettingUpdate(m.chat, isClose).then(async (_)=> {
   ...
   m.reply(❱❱ 𝗢𝗥𝗗𝗘𝗡𝗘𝗦 𝗥𝗘𝗖𝗜𝗕𝗜𝗗𝗔𝗦 ❰❰\nEste grupo estara ${isClose == 'announcement' ? 'cerrado' : 'abierto'} ${args[1] ? durante *${clockString(timeoutset)} Horas : ''});
  });
  if (args[1]) {
  setTimeout(async () => {
...
handler.help = ['gc cerrargrupoen/abrirgrupoen *<número>*'];
handler.tags = ['group'];
handler.command = /^(group|gctime|gc)$/i;
