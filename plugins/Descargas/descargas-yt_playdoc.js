...
const handler = async (m, {conn, command, args, text, usedPrefix}) => {
  ...
  try {
    ...
    let additionalText = '';
    if (command === 'play3doc' || command == 'playdoc') {
      additionalText = 'audio 🔊';
    } else if (command === 'play4doc' || command == 'playdoc2') {
...
» ${yt_play[0].url}
﹘﹘﹘﹘﹘﹘﹘﹘﹘﹘﹘﹘
𝙀𝙉𝙑𝙄𝘼𝙉𝘿𝙊 ${additionalText}, 𝙀𝙎𝙋𝙀𝙍𝙀 𝙐𝙉 𝙈𝙊𝙈𝙀𝙉𝙏𝙊...
*⇄ㅤ     ◁   ㅤ  ❚❚ㅤ     ▷ㅤ     ↻*`.trim();
    ...
    if (command == 'play4doc' || command == 'playdoc2') {
      try {
        const qu = '360';
        const q = qu + 'p';
        const v = yt_play[0].url;
        ...
        await await conn.sendMessage(m.chat, {document: {url: dl_url}, fileName: `${ttl}.mp4`, mimetype: 'video/mp4', caption: `🌸 𝗧𝗜𝗧𝗨𝗟𝗢: ${ttl}\n✅ 𝗣𝗘𝗦𝗢 𝗗𝗘𝗟 𝗩𝗜𝗗𝗘𝗢: ${size}`, thumbnail: await fetch(yt.thumbnail)}, {quoted: m});
      } catch {
        try {
...
handler.command = /^(playdoc|playdoc2|play3doc|play4doc)$/i;
